...
public class DispatchRoutingHelper {
  ...
  public static int getDestinationContext(HttpServletRequest request) {
    ...
    if (url.contains("/app")) {
      ...
      return SharedConstants.DISPATCH_CONTEXT_MAIN;
      // throw new
      // RuntimeException("Encountered an invalid destination context in main dispatch. The destination scope did not match any of the valid scopes: "+
      // url);
    }
  ...
  /**
   * Determine if we are requesting a 'registration' or any type: register/ register-trainer/
   * register-etc/
   * 
   * @param request
   * @return
   ...
   * 
   * 
   * If we have module specific data, we will need to revisit this structure, etc.
   * 
   * @param request
   * @return
   */
