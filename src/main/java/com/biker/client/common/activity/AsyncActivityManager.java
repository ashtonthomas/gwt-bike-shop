...
public class AsyncActivityManager implements PlaceChangeEvent.Handler, PlaceChangeRequestEvent.Handler {
	...
	public void onPlaceChange(PlaceChangeEvent event) {
		...
		if (event.getNewPlace() instanceof AppPlace) {
			...
			getNextActivity(event, new ActivityCallbackHandler<AppActivity>() {
				...
				@Override
				public void onRecieveActivity(AppActivity nextActivity,
						AppPlace linkedPlace) {
					...
					if (currentPlace != linkedPlace) {
						...
						/*
						 * Kill off the activity's handlers, so it doesn't have
						 * to worry about them accidentally firing as a side
						 * effect of its tear down
						 */
	...
	private void getNextActivity(PlaceChangeEvent event,
		...
		mapper.getActivity((AppPlace) event.getNewPlace(),
