...
public class MainAsyncActivityMapper extends AsyncActivityMapper<AppPlace, AppActivity> {
  ...
  public void getActivity(final AppPlace place,
    ...
    if (place instanceof InventoryPlace) {
      ...
      activityCallbackHandler.onRecieveActivity(factory.messagesActivity((InventoryPlace) place),
          place);

    } else if (place instanceof AssociatesPlace) {

      GWT.runAsync(AssociatesActivity.class, new RunAsyncCallback() {
        @Override
      ...
      GWT.runAsync(ShopSettingsActivity.class, new RunAsyncCallback() {
        @Override
        public void onSuccess() {
          activityCallbackHandler.onRecieveActivity(
      ...
      GWT.runAsync(ProfileActivity.class, new RunAsyncCallback() {
        @Override
        public void onSuccess() {
          activityCallbackHandler.onRecieveActivity(factory.profileActivity((ProfilePlace) place),
              place);
        }
